...
- **Tools:**
  ...
  - [`strategic_memory`](./docs/strategic_memory.md) — episodic store that `ReasoningKernel` can query and update
- **Client examples:**
...
### Migrating to MetaRouter

The previous `Router` implementation is deprecated in favor of `MetaRouter`,
which routes requests to registered experts based on task, context, and goal
metadata.
...
#### Using with `Planner` and `ReasoningKernel`
...
This snippet replaces the old router by initializing `MetaRouter`, registering
experts, and passing it to `ReasoningKernel` to execute plans generated by
`Planner`.

`ReasoningKernel` also accepts an optional `StrategicMemory` instance to
persist `Episode` objects after each step or token and to retrieve past
experiences before producing new tokens:

```python
from gpt_oss.strategic_memory import StrategicMemory

memory = StrategicMemory()
kernel = ReasoningKernel(router, memory=memory)
```

For an overview of goal management and mode activation, see
[the Planner documentation](./docs/planner.md).
